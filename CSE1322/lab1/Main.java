public class Main {
    ...
    public static void print_char(char[][] car, char[][] car2) { //2nd print char constructor, takes two inputs and prints them side by side
        for (int i = 0; i < car.length; i++) {
            for (int j = 0; j < car[0].length*2; j++) {
                if(j < car[0].length) { //separates the inputs into two different printers to print line by line
                    System.out.print(car[i][j]);
                }
                else {
    ...
    public static char[][] make_forward() { //copy-pasted make forward method for the assignment
